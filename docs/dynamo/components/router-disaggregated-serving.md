...
For the high-level deployment matrix, see [Router Guide](/dynamo/user-guides/kv-cache-aware-routing). For the router flags used in this setup, see [Configuration and Tuning](/dynamo/components/router/configuration-and-tuning).

If prefill and decode workers span topology domains such as zones or racks, use [Topology-Aware KV Transfer](/dynamo/components/router/topology-aware-kv-transfer) to constrain or bias decode routing toward workers in the selected prefill worker's transfer domain.
