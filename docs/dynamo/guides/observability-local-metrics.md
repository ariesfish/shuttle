...
**For creating custom metrics**, see the [Metrics Developer Guide](/dynamo/user-guides/observability-local/metrics-developer-guide).
...
- **Engine Metrics** (Pass-through) - Backend engines expose their own metrics: [vLLM](/dynamo/backends/v-llm/observability) (`vllm:*`), [SGLang](/dynamo/backends/sg-lang/observability) (`sglang:*`), [TensorRT-LLM](/dynamo/backends/tensor-rt-llm/observability) (`trtllm_*`)
...
For router deployment modes, see the [Router Guide](/dynamo/user-guides/kv-cache-aware-routing). For router flags and tuning, see [Configuration and Tuning](/dynamo/components/router/configuration-and-tuning).
...
For the full list of metrics, configuration options, and architecture details, see the upstream [NIXL Telemetry documentation](https://github.com/ai-dynamo/nixl/blob/main/docs/telemetry.md) and [Prometheus exporter README](https://github.com/ai-dynamo/nixl/blob/main/src/plugins/telemetry/prometheus/README.md). For Kubernetes, see [Enable NIXL Telemetry](/dynamo/kubernetes-deployment/observability-k-8-s/metrics#enable-nixl-telemetry-optional).
...
- [Backend Guide](/dynamo/user-guides/writing-python-workers-lower-level)
- [Forward Pass Metrics (SGLang)](/dynamo/backends/sg-lang/observability#forward-pass-metrics-fpm) — Per-iteration scheduler telemetry via ZMQ/NATS for planner-driven scaling (intended architecture; not available in the 1.2.0 SGLang runtime image)
